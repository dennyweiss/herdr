...
`show_tabs` 会将每个 Space 的标签页作为缩进行列在 Space 条目下方，使用与 worktree 子项相同的 `├─` 树形样式。当 Space 有多个标签页或有自定义名称的标签页时才会显示。每个标签页行显示该标签页的聚合代理状态和标签。点击标签页即可聚焦，需要时侧边栏会切换 Space。右键打开标签页菜单。Space 行末尾的折叠符号或 Space 上下文菜单可以折叠该 Space 的标签页列表；折叠状态按客户端记忆。默认值为 `false`。
