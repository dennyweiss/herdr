...
`show_tabs` は、各 Space のタブを Space エントリの下にインデントされた行として表示します。worktree の子と同じ `├─` ツリー表示を使います。Space に 2 つ以上のタブ、またはカスタム名のタブがある場合に表示されます。各タブ行にはタブのエージェント状態とラベルが表示されます。タブをクリックするとフォーカスが移り、必要に応じて Space も切り替わります。右クリックでタブメニューが開きます。Space 行末尾のシェブロン、または Space のコンテキストメニューでタブ一覧を折りたためます。折りたたみ状態はクライアントごとに記憶されます。デフォルトは `false` です。
