...
`show_tabs` lists each Space's tabs as indented rows under the Space entry, using the same `├─` tree style as worktree children. Rows appear when a Space has more than one tab or a custom-named tab. Each tab row shows the tab's rolled-up agent state and label. Click a tab to focus it; the sidebar switches Space when needed. Right-click opens the tab menu. The chevron at the end of the Space row, or the Space context menu, folds that Space's tab list; folded lists are remembered per client. It defaults to `false`.
